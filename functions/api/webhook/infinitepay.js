 ...
 * Recebe notificação InfinitePay e libera/revoga acesso.
 */
...
export async function onRequestPost({ request, env }) {
  ...
  const externalRef = String(body?.reference ?? body?.external_reference ?? body?.product_id ?? "");
  ...
  const mapRes = await fetch(
    `${SB}/rest/v1/clube_produto_plataformas?plataforma=eq.infinitepay&plataforma_produto_id=eq.${encodeURIComponent(externalRef)}&select=produto_id`,
    { headers: sbH }
  );
  ...
  const aprovados = ["approved", "paid", "succeeded", "captured", "active"];
  const cancelados = ["refunded", "reversed", "cancelled", "canceled", "chargeback"];

  if (aprovados.some(s => status.toLowerCase().includes(s))) {
    await upsertAcesso(userId, produtoId, orderId, "infinitepay", true, sbH, SB);
    return resp({ ok: true });
  }
  if (cancelados.some(s => status.toLowerCase().includes(s))) {
    await upsertAcesso(userId, produtoId, orderId, "infinitepay", false, sbH, SB);
    return resp({ ok: true });
  }
...
async function getUserIdSimple(email, sbH, SB, env) {
  ...
  const r = await fetch(`${SB}/auth/v1/admin/users`, {
    method: "POST",
    headers: sbH,
    body: JSON.stringify({ email, password: Math.random().toString(36).slice(2, 10) + "A1!", email_confirm: true, user_metadata: { nome: email.split("@")[0] } }),
  });
  const d = await r.json();
  if (d?.id) {
